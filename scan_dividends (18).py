...
def load_config():
    ...
    if not os.path.exists(CONFIG_PATH):
        raise FileNotFoundError(
            ...
            f"{CONFIG_PATH} gefunden. Lokal: config.json anlegen mit den 7 API-Keys. "
            f"In GitHub Actions: die 7 Secrets hinterlegen, siehe README."
        )
...
def fetch_fundamentals_brapi(ticker, api_key):
    ...
    ohne '.SA'-Endung (z.B. 'PETR4' statt 'PETR4.SA'). Noch nicht mit
    echtem Key getestet."""
    if not api_key:
        return None
    brapi_symbol = ticker[:-3] if ticker.endswith(".SA") else ticker
    data = api_get(f"quote/{brapi_symbol}", api_key, base_url=BRAPI_URL,
                    key_param="token", source_label="brapi.dev")
    results = (data or {}).get("results") if isinstance(data, dict) else None
    if not results:
        log(f"  brapi.dev-Fehler bei {brapi_symbol}: keine verwertbare Antwort")
        return None
    r = results[0]
    dividend_yield_pct = r.get("dividendYield")
...
def fetch_fundamentals(ticker, fmp_key, twelve_data_key, finnhub_key,
                        alpha_vantage_key=None, marketstack_key=None, leeway_key=None, brapi_key=None):
...
def fetch_price_history_stooq(ticker):
    ...
    for line in lines[1:]:
        ...
        if len(parts) >= 5:
            try:
                rows.append(float(parts[4]))  # Spalte 5 = Schlusskurs
            except ValueError:
                continue
    if not rows:
        return None
    ...
    def pct_change(handelstage):
        idx = max(0, len(rows) - 1 - handelstage)
        old = rows[idx]
...
def main():
    ...
    for ticker in all_tickers:
        ...
        data = fetch_fundamentals(ticker, fmp_key, twelve_data_key, finnhub_key,
                                   alpha_vantage_key, marketstack_key, leeway_key, brapi_key)
        if data is None:
        ...
        if data["evaluation"]["meets_min_yield"] and fmp_key:
            # Kursentwicklung (1M/3M/1J) + ISIN + News-Sentiment nur fuer
            # Aktien holen, die ohnehin im Scanner erscheinen (>= Mindest-
            # rendite) - spart Kontingent bei den vielen Aktien, die
            # sowieso ausgefiltert werden.
        ...
        if data["evaluation"]["meets_min_yield"] and (data.get("price") is None or data.get("perf_1y") is None):
            # Stooq als Kurs-Ergaenzung, v.a. fuer manuell recherchierte
            # Eintraege (kein Live-Kurs) oder wenn FMP keine Kursentwicklung
            # liefern konnte - braucht keinen API-Key.
            stooq_data = fetch_price_history_stooq(ticker)
            if stooq_data:
                if data.get("price") is None:
                    data["price"] = stooq_data["price"]
                for feld in ("perf_1m", "perf_3m", "perf_1y"):
                    if data.get(feld) is None:
                        data[feld] = stooq_data[feld]
                log(f"    -> Kurs/Kursverlauf via Stooq ergaenzt")
...
const TRAP_ICON = `<svg width="26" height="26" viewBox="0 0 24 24" style="vertical-align:middle;margin-left:4px;" title="Moegliche Dividenden-Falle">
  ...
  <text x="12" y="18.5" text-anchor="middle" fill="#FF2E9A" font-size="14" font-weight="800" font-family="Arial,sans-serif">!</text>
</svg>`;
