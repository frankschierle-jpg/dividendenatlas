...
def fetch_price_history_yahoo(ticker):
    ...
    url = f"{YAHOO_CHART_URL}/{ticker}?range=3y&interval=1d"
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
    try:
        with urllib.request.urlopen(req, timeout=20) as resp:
            status = resp.status
...
def main():
    ...
    for ticker in all_tickers:
        ...
        dividend_per_share = stamm.get("dividend_per_share")
        ...
        if dividend_per_share and price:
            ...
            if dividend_yield_pct is not None:
                log(f"    -> WARNUNG: keine dividend_per_share in stammdaten.json - "
                    f"Rendite {dividend_yield_pct}% ist ein EINGEFRORENER Startwert "
                    f"(Stand {stamm.get('stand') or 'unbekannt'}), NICHT tagesaktuell.")
        ...
        data = {
            ...
            "next_payout": stamm.get("next_payout"),
            "next_payout_hinweis": stamm.get("next_payout_hinweis"),
            "data_source": quelle,
...
<div class="hint">Nur Inspiration, komplett getrennt vom Dividendendepot. Zeigt ausschliesslich Aktien mit Rendite ≥ {MIN_YIELD_PCT:.0f}%. Klick auf eine Zeile fuer das vollstaendige Profil. Klick auf eine Spaltenüberschrift zum Sortieren.
  <br><span style="color:var(--red);">⚠ hinter einer Rendite heisst: fuer diese Aktie ist keine Dividende je Aktie hinterlegt, die Rendite ist ein
  eingefrorener Startwert und bewegt sich nicht mit dem Kurs. Im Profil die Dividende je Aktie eintragen - danach rechnet der taegliche Lauf sie korrekt.</span></div>
...
// Sagt im Klartext, ob die angezeigte Rendite tagesaktuell berechnet ist
// oder nur ein alter, eingefrorener Startwert - und was dagegen zu tun ist.
function renditeHerkunftHinweis(r) {{
  ...
  if (eigen) {{
    return `<div style="font-size:11px;color:#B45309;font-weight:normal;margin-top:2px;">
      ✏️ Von dir geaendert - aus ${{fmt(r.dividend_per_share)}} ${{waehrungFuer(r.ticker)}} Dividende je Aktie
      ÷ Kurs ${{fmt(r.price)}} ${{waehrungFuer(r.ticker)}}. Wird beim naechsten Lauf mit dem dann aktuellen Kurs neu gerechnet.</div>`;
  }}
  if (q === 'berechnet') {{
    return `<div style="font-size:11px;color:var(--green);font-weight:normal;margin-top:2px;">
      ✓ Tagesaktuell: ${{fmt(r.dividend_per_share)}} ${{waehrungFuer(r.ticker)}} Dividende je Aktie
      ÷ Kurs ${{fmt(r.price)}} ${{waehrungFuer(r.ticker)}} vom letzten Datenabruf.${{r.dividend_quelle ? ' ' + r.dividend_quelle : ''}}</div>`;
  }}
  if (q === 'berechnet_alter_kurs') {{
    return `<div style="font-size:11px;color:#B45309;font-weight:normal;margin-top:2px;">
      ⚡ Aus ${{fmt(r.dividend_per_share)}} Dividende je Aktie ÷ <strong>letztem bekannten</strong> Kurs berechnet -
      an diesem Tag lieferte keine Kursquelle einen frischen Kurs.</div>`;
  }}
  if (q === 'startwert') {{
    return `<div style="font-size:11px;color:var(--red);font-weight:600;margin-top:2px;">
      ⚠ <strong>Nicht tagesaktuell.</strong> Fuer diese Aktie ist keine Dividende je Aktie hinterlegt, deshalb steht hier
      ein fest eingetragener Startwert${{r.stammdaten_stand ? ' (Stand ' + r.stammdaten_stand + ')' : ''}}, der sich mit dem Kurs NICHT mitbewegt.
      <span style="font-weight:400;">Trag oben die Dividende je Aktie ein - danach wird die Rendite jeden Tag automatisch
      neu berechnet und stimmt wieder.</span></div>`;
  }}
  return `<div style="font-size:11px;color:var(--muted);font-weight:normal;margin-top:2px;">
    Keine Renditequelle hinterlegt - trag oben die Dividende je Aktie ein.</div>`;
}}
    ...
    <table>
      ...
      <tr><td>Dividende je Aktie (jaehrlich)</td><td>${{editableCell(r.ticker, 'dividend_per_share', r.dividend_per_share)}} <span style="color:var(--muted);">${{waehrungFuer(r.ticker)}}</span>
        ...
        <div style="font-size:11px;color:var(--muted);font-weight:normal;margin-top:2px;">
          <strong>Anklicken und aendern.</strong> Dieser Wert ist die Grundlage der Rendite: Der taegliche Lauf rechnet
          jeden Morgen neu <em>Dividende ÷ aktueller Kurs</em>. Wird dauerhaft in stammdaten.json gespeichert und stoesst
          direkt danach einen neuen Lauf an - Rendite, Netto-Rendite und Risikostufe aktualisieren sich also von selbst.
          Leer lassen (Feld leeren + Enter) setzt die eigene Aenderung wieder zurueck.
          ${{r.dividend_quelle ? 'Quelle bisher: ' + r.dividend_quelle : ''}}
        </div></td></tr>
...
function renderScanner() {{
  ...
  filtered.forEach((r, i) => {{
    ...
    tr.innerHTML = `
      ...
      <td class="yield ${{of_.includes('dividend_yield_pct') ? 'overridden' : ''}}" title="${{r.dividend_yield_quelle === 'startwert' ? 'Eingefrorener Startwert - bewegt sich nicht mit dem Kurs. Zeile anklicken und im Steckbrief die Dividende je Aktie eintragen.' : 'Taeglich aus Dividende je Aktie und Kurs berechnet'}}">${{editableCell(r.ticker, 'dividend_yield_pct', r.dividend_yield_pct)}}${{r.dividend_yield_quelle === 'startwert' && !of_.includes('dividend_yield_pct') && !of_.includes('dividend_per_share') ? '<span style="color:var(--red);font-size:11px;margin-left:3px;cursor:help;">⚠</span>' : ''}}</td>
