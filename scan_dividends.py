...
CANDIDATE_UNIVERSE = [
    ...
    ("O2D.DE", "Deutschland", "Telekom"), ("VNA.DE", "Deutschland", "Immobilien"),
...
MANUAL_RESEARCH_DATA = {
    ...
    "VOW3.DE": {"name": "Volkswagen Vz.", "dividend_yield_pct": 7.44, "payout_ratio_pct": 31.37,
                "quelle": "finanzen100.de", "stand": "2026-08"},
    "O2D.DE": {"name": "Telefonica Deutschland", "dividend_yield_pct": 7.13, "payout_ratio_pct": -737.62,
               "quelle": "finanzen100.de - ACHTUNG unplausible Ausschuett., moeglw. delisted", "stand": "2026-08"},
    ...
    "PAH3.DE": {"name": "Porsche Automobil Holding", "dividend_yield_pct": 6.06, "payout_ratio_pct": 22.65,
                "quelle": "finanzen100.de", "stand": "2026-08"},
    "DWNI.DE": {"name": "Deutsche Wohnen", "dividend_yield_pct": 5.91, "payout_ratio_pct": 46.17,
                "quelle": "finanzen100.de - moeglw. delisted (Squeeze-out)", "stand": "2026-08"},
    ...
    "NDA-FI.HE": {"name": "Nordea Bank Abp", "dividend_yield_pct": 8.52, "payout_ratio_pct": 64.7,
                  "quelle": "Simply Wall St", "stand": "2026-08"},
    ...
    "BMW.DE": {"name": "BMW", "dividend_yield_pct": 7.5, "payout_ratio_pct": None,
               "quelle": "Nutzer-Recherche (aktienfinder.net u.a.)", "stand": "2026-08"},
    "DTG.DE": {"name": "Daimler Truck Holding", "dividend_yield_pct": 5.1, "payout_ratio_pct": None,
               "quelle": "Nutzer-Recherche", "stand": "2026-08"},
    "CON.DE": {"name": "Continental", "dividend_yield_pct": 5.0, "payout_ratio_pct": None,
               "quelle": "Nutzer-Recherche", "stand": "2026-08"},
    "SDF.DE": {"name": "K+S", "dividend_yield_pct": 5.4, "payout_ratio_pct": None,
               "quelle": "Nutzer-Recherche", "stand": "2026-08"},
    "EVK.DE": {"name": "Evonik Industries", "dividend_yield_pct": 9.2, "payout_ratio_pct": None,
               "quelle": "Nutzer-Recherche - sehr hohe Rendite, genau pruefen", "stand": "2026-08"},
    "WCH.DE": {"name": "Wacker Chemie", "dividend_yield_pct": 5.2, "payout_ratio_pct": None,
               "quelle": "Nutzer-Recherche", "stand": "2026-08"},
    ...
    "KIM": {"name": "Kimco Realty", "dividend_yield_pct": 5.2, "payout_ratio_pct": None,
            "quelle": "Nutzer-Recherche", "stand": "2026-08"},
    "TRP": {"name": "TC Energy", "dividend_yield_pct": 6.3, "payout_ratio_pct": None,
            "quelle": "Nutzer-Recherche", "stand": "2026-08"},
    "BCE": {"name": "BCE Inc.", "dividend_yield_pct": 8.7, "payout_ratio_pct": None,
            "quelle": "Nutzer-Recherche - sehr hohe Rendite, genau pruefen", "stand": "2026-08"},
    "TU": {"name": "Telus", "dividend_yield_pct": 6.8, "payout_ratio_pct": None,
           "quelle": "Nutzer-Recherche", "stand": "2026-08"},
    "BNS": {"name": "Bank of Nova Scotia", "dividend_yield_pct": 5.9, "payout_ratio_pct": None,
            "quelle": "Nutzer-Recherche", "stand": "2026-08"},
    ...
    "PBA": {"name": "Pembina Pipeline", "dividend_yield_pct": 5.6, "payout_ratio_pct": None,
            "quelle": "Nutzer-Recherche", "stand": "2026-08"},
    "LGEN.L": {"name": "Legal & General", "dividend_yield_pct": 8.6, "payout_ratio_pct": None,
               "quelle": "Nutzer-Recherche", "stand": "2026-08"},
    ...
    "MNG.L": {"name": "M&G plc", "dividend_yield_pct": 9.1, "payout_ratio_pct": None,
              "quelle": "Nutzer-Recherche - sehr hohe Rendite, genau pruefen", "stand": "2026-08"},
    "HSBA.L": {"name": "HSBC Holdings", "dividend_yield_pct": 6.4, "payout_ratio_pct": None,
               "quelle": "Nutzer-Recherche", "stand": "2026-08"},
    "GLEN.L": {"name": "Glencore", "dividend_yield_pct": 5.2, "payout_ratio_pct": None,
               "quelle": "Nutzer-Recherche", "stand": "2026-08"},
...
def fetch_fundamentals_manual_research(ticker):
    ...
    aber Rendite und Risiko-Bewertung funktionieren normal."""
...
def fetch_fundamentals(ticker, fmp_key, twelve_data_key, finnhub_key,
    ...
    """Versucht der Reihe nach FMP -> Twelve Data -> Finnhub -> Alpha
    Vantage -> Marketstack -> manuelle Recherche-Datenbank. So muss vorher
    nicht bekannt sein, welcher Anbieter welchen Ticker abdeckt - das
    Skript findet es pro Aktie selbst heraus, indem es einfach die
    ...
    if result is not None:
        ...
        return result
    result = fetch_fundamentals_alphavantage(ticker, alpha_vantage_key)
    if result is not None:
        log(f"    -> ueber Alpha Vantage erhalten")
        return result
    result = fetch_fundamentals_marketstack(ticker, marketstack_key)
    if result is not None:
        log(f"    -> ueber Marketstack erhalten")
        return result
...
def main():
    ...
    if not any([fmp_key, twelve_data_key, finnhub_key, alpha_vantage_key, marketstack_key]):
...
#scanner-tbl th[data-key]:hover{{color:var(--blueB);}}
.editable-value{{cursor:text;border-bottom:1px dotted var(--muted);}}
...
<div class="meta">Stand: <span id="stand"></span> · <a href="recherche.html" style="color:var(--blue);">→ Recherche-Quellen</a></div>
...
<div class="controls" style="margin-bottom:10px;">
  <button class="risk-filter-btn active" data-risk="alle" onclick="setRiskFilter('alle')">Alle Risiken</button>
...
<div class="controls" style="margin-bottom:14px;">
  <select id="land-filter" onchange="setLandFilter(this.value)" style="padding:6px 10px;border-radius:20px;border:1px solid var(--line);font-size:12px;margin-right:8px;">
...
// Naeherungswerte fuer Waehrungsumrechnung zu EUR, Stand August 2026.
// Depot wird IMMER in EUR gefuehrt - gelegentlich manuell aktualisieren,
// falls sich Wechselkurse deutlich verschieben.
const FX_RATES_TO_EUR = {{
  EUR: 1.0, USD: 0.92, GBP: 1.17, CHF: 1.06, JPY: 0.0061, HKD: 0.118, SGD: 0.685
}};
...
let riskFilter = 'alle', landFilter = 'alle', sektorFilter = 'alle';
...
function applyScannerOverrides(baseData) {{
  ...
  return baseData.map(r => {{
    ...
    if (!ov || Object.keys(ov).length === 0) return {{...r, _overriddenFields: []}};
    return {{...r, ...ov, _overriddenFields: Object.keys(ov)}};
  }});
}}
...
function evaluateScannerCandidateJS(f) {{
  ...
  else if (payout !== null && payout !== undefined && payout <= 60 && (debt === null || debt === undefined || debt <= 1.0)) risk = 'niedrig';
  else if (payout !== null && payout !== undefined && payout <= 85 && (debt === null || debt === undefined || debt <= 2.0)) risk = 'mittel';
  else risk = 'hoch';
  return {{
    trap_warning: trapWarning, risk,
...
function saveEditCell(input, ticker, field) {{
  ...
  if (val === '') delete overrides[ticker][field];
  else overrides[ticker][field] = parseFloat(val);
...
function renderScanner() {{
  ...
  let filtered = dataWithOverrides.filter(r =>
    (riskFilter === 'alle' || r.evaluation.risk === riskFilter) &&
  ...
  filtered.forEach((r, i) => {{
    ...
    tr.innerHTML = `
      ...
      <td class="${{of_.includes('perf_1m') ? 'overridden' : ''}}">${{editableCell(r.ticker, 'perf_1m', r.perf_1m)}}</td>
      <td class="${{of_.includes('perf_3m') ? 'overridden' : ''}}">${{editableCell(r.ticker, 'perf_3m', r.perf_3m)}}</td>
      <td class="${{of_.includes('perf_1y') ? 'overridden' : ''}}">${{editableCell(r.ticker, 'perf_1y', r.perf_1y)}}</td>
      <td class="${{of_.includes('payout_ratio_pct') ? 'overridden' : ''}}">${{editableCell(r.ticker, 'payout_ratio_pct', r.payout_ratio_pct)}}</td>
      <td class="${{of_.includes('debt_to_equity') ? 'overridden' : ''}}">${{editableCell(r.ticker, 'debt_to_equity', r.debt_to_equity)}}</td>
      <td>
