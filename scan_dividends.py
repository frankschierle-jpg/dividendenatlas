...
function showDetail(ticker) {{
  ...
  document.getElementById('detail-content').innerHTML = `
    ...
    <table>
      ...
      <tr><td>Dividendenrendite (brutto)</td><td>${{fmt(r.dividend_yield_pct, '%')}}</td></tr>
      ...
      <tr><td>Ausschuettungsquote${{r.payout_ratio_fcf_pct !== null && r.payout_ratio_fcf_pct !== undefined ? ' (Free Cashflow)' : ' (Gewinn)'}}</td><td>${{fmt(r.payout_ratio_fcf_pct !== null && r.payout_ratio_fcf_pct !== undefined ? r.payout_ratio_fcf_pct : r.payout_ratio_pct, '%')}}
        <div style="font-size:11px;color:var(--muted);font-weight:normal;margin-top:2px;">${{r.payout_ratio_fcf_pct !== null && r.payout_ratio_fcf_pct !== undefined ? 'FCF-basiert - robuster gegen Bilanzeffekte (Abschreibungen, Fair-Value-Anpassungen) als die gewinnbasierte Quote.' : 'Gewinnbasiert - bei Immobilien/Versicherern durch Bilanzeffekte teils verzerrt.'}}</div></td></tr>
