...
RISIKO_MAX = {"quote": 40, "verschuldung": 25, "dividende": 20, "rendite": 15}
...
def risiko_punkte(payout, debt, yield_pct, dividend_auto, is_reit, quote_ist_fcf,
    ...
    if de_ausnahme:
        ...
        p = _spanne(debt, 0, de_niedrig, 0, 6)
        text = f"Verschuldung D/E {debt} liegt unter der Sektorschwelle {de_niedrig}."
    elif debt <= de_mittel:
        p = _spanne(debt, de_niedrig, de_mittel, 6, 16)
        text = f"Verschuldung D/E {debt} liegt zwischen {de_niedrig} und {de_mittel}."
    else:
        p = _spanne(debt, de_mittel, de_mittel * 2, 16, 25)
        text = f"Verschuldung D/E {debt} liegt ueber der Sektorschwelle {de_mittel}."
    ...
    if not auto.get("vergleichbar") or veraenderung is None:
        ...
        p = _spanne(-veraenderung, 5, 20, 8, 18)
...
const RISIKO_MAX_JS = {{quote: 40, verschuldung: 25, dividende: 20, rendite: 15}};
...
function risikoPunkteJS(payout, debt, yieldPct, dividendAuto, isReit, quoteIstFcf,
  ...
  if (isReit && !quoteIstFcf) {{
    if (hat(payout) && payout > 300) {{
      p = 40; text = `Gewinnbasierte Quote ${{payout.toFixed(0)}}% - auch fuer eine Immobilienaktie zu hoch.`;
    }} else {{
      p = 12; luecken += p;
  ...
  if (deAusnahme) {{
    ...
    p = spanneJS(debt, 0, deNiedrig, 0, 6);
    text = `Verschuldung D/E ${{debt}} liegt unter der Sektorschwelle ${{deNiedrig}}.`;
  }} else if (debt <= deMittel) {{
    p = spanneJS(debt, deNiedrig, deMittel, 6, 16);
    text = `Verschuldung D/E ${{debt}} liegt zwischen ${{deNiedrig}} und ${{deMittel}}.`;
  }} else {{
    p = spanneJS(debt, deMittel, deMittel * 2, 16, 25);
    text = `Verschuldung D/E ${{debt}} liegt ueber der Sektorschwelle ${{deMittel}}.`;
  }}
  ...
  if (!auto.vergleichbar || !hat(v)) {{
    ...
    p = 20; text = `Dividende gegenueber dem Vorjahreszeitraum um ${{Math.abs(v).toFixed(0)}}% GEKUERZT.`;
  }} else if (v < -5) {{
    p = spanneJS(-v, 5, 20, 8, 18);
    text = `Dividende gegenueber dem Vorjahreszeitraum um ${{Math.abs(v).toFixed(0)}}% gesunken.`;
  }} else if (v <= 5) {{
    p = 6; text = `Dividende gegenueber dem Vorjahreszeitraum weitgehend stabil (${{v > 0 ? '+' : ''}}${{v.toFixed(0)}}%).`;
  }} else {{
    p = 2; text = `Dividende gegenueber dem Vorjahreszeitraum um ${{v.toFixed(0)}}% erhoeht.`;
  }}
  ...
  else if (yieldPct > 12) {{ p = 15; text = `Rendite ${{yieldPct.toFixed(1)}}% - so hoch, dass der Markt offensichtlich eine Kuerzung erwartet.`; }}
  else if (yieldPct > 9) {{ p = 11; text = `Rendite ${{yieldPct.toFixed(1)}}% liegt deutlich ueber dem Ueblichen.`; }}
  else if (yieldPct > 7) {{ p = 6; text = `Rendite ${{yieldPct.toFixed(1)}}% liegt spuerbar ueber dem Ueblichen.`; }}
  else if (yieldPct > 5) {{ p = 2; text = `Rendite ${{yieldPct.toFixed(1)}}% ist erhoeht, aber unauffaellig.`; }}
