...
def fetch_fundamentals_twelvedata(ticker, api_key):
    ...
    if not stat:
        # /statistics ist im kostenlosen Twelve-Data-Plan gesperrt (403) -
        # ohne echte Kennzahlen ist dieser "Erfolg" nutzlos (nur Kurs, keine
        # Ausschuettungsquote/Verschuldung). Lieber ehrlich scheitern und
        # die naechste Quelle (Finnhub) versuchen lassen, statt falsche
        # 0%-Werte als echtes Ergebnis auszugeben.
        log(f"  Twelve Data lieferte nur Kurs, keine Kennzahlen fuer {td_symbol} "
...
def main():
    ...
    for ticker in all_tickers:
        ...
        if data["evaluation"]["meets_min_yield"] and fmp_key:
            # Kursentwicklung (1M/3M/1J) nur fuer Aktien holen, die ohnehin
            # im Scanner erscheinen (>= Mindestrendite) - spart Kontingent
            # bei den vielen Aktien, die sowieso ausgefiltert werden.
    ...
    scanner_view = [history[t] for t in scanner_tickers
                    if t in history and history[t]["evaluation"]["meets_min_yield"]]
    scanner_view.sort(key=lambda e: (RISK_ORDER[e["evaluation"]["risk"]],
...
<h2>Watchlist (nach Risiko sortiert, {len(scanner_view)} Kandidaten mit ≥{MIN_YIELD_PCT:.0f}% Rendite)</h2>
<div class="hint">Nur Inspiration, komplett getrennt vom Dividendendepot. Zeigt ausschliesslich Aktien mit Rendite ≥ {MIN_YIELD_PCT:.0f}%, sortiert nach Risiko (niedrig zuerst). Klick auf eine Zeile fuer das vollstaendige Profil.</div>
