...
AUTO_DISCOVER_MAX_NEW = 12  # neue, bisher unbekannte Ticker pro Lauf, die
# komplett durch die Anreicherungs-Pipeline laufen (Kontingent!). Bewusst
# klein gehalten - jeder neue Ticker kostet bis zu ~5 FMP-Calls (Fundamental-
# daten + Kursentwicklung + ISIN + News + Ratings/Zahltermin), falls er die
# Mindestrendite erreicht. Hochsetzen, wenn das Kontingent es hergibt.


def fetch_screener_candidates(api_key, limit=250):
...
def main():
    ...
    for ticker in all_tickers:
        ...
        if (data["evaluation"]["meets_min_yield"] or ticker in depot_tickers) and fmp_key:
            ...
            # FMP-Kontingent (250/Tag) im Auge behalten; falls es zu oft
...
function buildGradesSection(grades) {{
  ...
  const items = grades.map(g => `
    <div style="padding:8px 0;border-bottom:1px solid var(--line);font-size:12px;">
      ...
      <div style="color:var(--muted);margin-top:2px;">${{g.previous_grade || '?'}} → ${{g.new_grade || '?'}} <span style="font-size:10px;">(${{g.date || ''}})</span></div>
    </div>`).join('');
