...
def evaluate_scanner_candidate(f):
    ...
    if trap_warning:
        ...
        # Ist auch die unbekannt, bleibt 'mittel' - es fehlt schlicht die
        # Grundlage fuer ein 'niedrig'.
        if debt is None:
            risk = "mittel"
            ...
            risk = "hoch"
    elif (payout is not None and payout <= niedrig_max) and (debt is None or debt <= de_niedrig):
        risk = "niedrig"
    elif (payout is not None and payout <= mittel_max) and (debt is None or debt <= de_mittel):
        risk = "mittel"
    else:
        risk = "hoch"
...
def main():
    ...
    finnhub_queue = set(payout_warteschlange(
        all_tickers, stammdaten, history, ("nordamerika",), PAYOUT_PRO_LAUF_FINNHUB)) if finnhub_key else set()
    leeway_queue = set(payout_warteschlange(
        all_tickers, stammdaten, history, ("europa", "asien"), PAYOUT_PRO_LAUF_LEEWAY)) if leeway_key else set()
    log(f"Ausschuettungsquote auffrischen: {len(finnhub_queue)} ueber Finnhub (USA/Kanada), "
        f"{len(leeway_queue)} ueber Leeway (Europa/Asien).")
    ...
    for ticker in all_tickers:
        ...
        if ticker in finnhub_queue:
            treffer = fetch_payout_finnhub(ticker, finnhub_key)
            if treffer:
                payout_auto = {"wert": treffer["payout_ratio_pct"], "quelle": "Finnhub",
                               "stand": datetime.now().strftime("%Y-%m-%d"),
                               "eps": treffer.get("eps"),
                               "debt_to_equity": treffer.get("debt_to_equity")}
                log(f"    -> Ausschuettungsquote {treffer['payout_ratio_pct']}% via Finnhub"
                    + (f", Verschuldung {treffer['debt_to_equity']}" if treffer.get("debt_to_equity") is not None else ""))
            time.sleep(0.3)   # Finnhub erlaubt 60 Anfragen/Minute
...
<div class="hint">Nur Inspiration, komplett getrennt vom Dividendendepot. Zeigt ausschliesslich Aktien mit Rendite ≥ {MIN_YIELD_PCT:.0f}%. Klick auf eine Zeile fuer das vollstaendige Profil. Klick auf eine Spaltenüberschrift zum Sortieren.
  ...
  Die Quoten werden reihum aufgefrischt, nach ein paar Laeufen sind alle erreichbaren echt.</div>
...
function evaluateScannerCandidateJS(f) {{
  ...
  if (trapWarning) risk = 'hoch';
    ...
    if (!hat(debt)) risk = 'mittel';
    else if (debt <= deNiedrig) risk = 'niedrig';
    else if (debt <= deMittel) risk = 'mittel';
    else risk = 'hoch';
  }}
  else if (hat(payout) && payout <= niedrigMax && (!hat(debt) || debt <= deNiedrig)) risk = 'niedrig';
  else if (hat(payout) && payout <= mittelMax && (!hat(debt) || debt <= deMittel)) risk = 'mittel';
  else risk = 'hoch';
  return {{
