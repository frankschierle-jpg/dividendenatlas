...
<div class="hint">Ticker, die Claude im Chat vorgeschlagen hat, aber noch nicht in der Watchlist stehen. Klick auf eine Zeile fuer den Steckbrief, dort direkt "In Watchlist übernehmen" klicken.</div>
<table id="vorschlaege-tbl">
  <thead><tr>
    <th>Ticker</th><th>Name</th><th>Land</th><th>Sektor</th><th>Dividende</th>
    <th>Rendite (geschätzt)</th><th>KGV</th><th>Marktkap.</th><th>Verschuldung</th>
    <th>ISIN</th><th>Nächste Auszahlung</th><th>Risiko</th><th></th>
  </tr></thead>
  <tbody id="vorschlaege-tbody"></tbody>
</table>''' if vorschlaege else ''}
...
function showDetail(ticker) {{
  const ausScanner = SCANNER.find(s => s.ticker === ticker);
  const base = ausScanner || VORSCHLAEGE.find(v => v.ticker === ticker);
  if (!base) return;
  const istVorschlag = !ausScanner;
  ...
  if ((r.dividend_per_share === undefined || r.dividend_per_share === null) && r.price && r.dividend_yield_pct != null) {{
    r.dividend_per_share = r.price * r.dividend_yield_pct / 100;
  }}
  if (istVorschlag && r.geschaetzte_rendite_pct != null && r.dividend_yield_pct == null) {{
    r.dividend_yield_pct = r.geschaetzte_rendite_pct;
  }}
  ...
  // der Browser nicht selbst machen kann). Bei Vorschlaegen gibt's noch
  // gar keine r.evaluation - {{...undefined}} ist in JS unproblematisch (= {{}}).
  ...
  document.getElementById('detail-content').innerHTML = `
    ...
    ${{istVorschlag ? `<div style="background:#FFFBEB;border:1px solid #FDE68A;border-radius:6px;padding:10px 12px;margin-bottom:10px;">
      <strong>🆕 Vorschlag von Claude</strong>${{r.grund ? `<div style="font-size:12px;margin-top:4px;">${{r.grund}}</div>` : ''}}
      <div style="margin-top:8px;"><button class="action-btn" onclick="vorschlagUebernehmenAusSteckbrief('${{r.ticker}}', this)">✓ In Watchlist übernehmen</button></div>
    </div>` : ''}}
    ...
    <table>
      ...
      <tr><td>Dividendenrendite (brutto)</td><td>${{fmt(r.dividend_yield_pct, '%')}}
        ${{r.dividend_per_share ? `<div style="font-size:11px;color:var(--muted);font-weight:normal;margin-top:2px;">Berechnet aus ${{fmt(r.dividend_per_share)}} Dividende/Aktie ÷ aktuellem Kurs.${{r.dividend_quelle ? ' ' + r.dividend_quelle : ''}}</div>` : ''}}</td></tr>
      <tr><td>Netto-Rendite (nach Quellensteuer)</td><td>${{fmt(r.netto_rendite_pct, '%')}}
      ...
      <tr><td>Gesamtrendite (3 Jahre)</td><td>${{fmt(r.gesamtrendite_3j_pct, '%')}}
...
async function speichereDauerhaft(ticker, felder, statusEl) {{
  ...
  try {{
    ...
    if (statusEl) {{
      if (data.workflow_ausgeloest === false) {{
        statusEl.textContent = '✓ Gespeichert, aber automatischer Neubau fehlgeschlagen - manuell "Run workflow" klicken (siehe Chat)';
        statusEl.style.color = '#B45309';
      }} else {{
        statusEl.textContent = '✓ Dauerhaft gespeichert - Seite baut sich in ca. 1-2 Min. automatisch neu';
...
function renderVorschlaege() {{
  const tbody = document.getElementById('vorschlaege-tbody');
  if (!tbody) return; // keine Vorschlaege vorhanden
  tbody.innerHTML = VORSCHLAEGE.map((v, i) => {{
    const evV = evaluateScannerCandidateJS({{...v, dividend_yield_pct: v.geschaetzte_rendite_pct}});
    const tr = `
    <tr onclick="showDetail('${{v.ticker}}')" style="cursor:pointer;background:#FFFBEB;">
      <td><strong>${{v.ticker}}</strong></td>
      <td>${{v.name || ''}}</td>
      <td>${{v.land || ''}}</td>
      <td>${{v.sektor || ''}}</td>
      <td>${{fmt(v.dividend_per_share)}}</td>
      <td class="yield">${{fmt(v.geschaetzte_rendite_pct, '%')}}</td>
      <td>${{fmt(v.pe_ratio)}}</td>
      <td>${{v.market_cap ? (v.market_cap/1e9).toFixed(1) + ' Mrd. $' : '–'}}</td>
      <td>${{fmt(v.debt_to_equity)}}</td>
      <td style="font-family:monospace;font-size:11px;">${{v.isin || '–'}}</td>
      <td>${{v.next_payout || '–'}}</td>
      <td><span class="badge ${{evV.risk}}">${{evV.risk}}</span></td>
      <td>
        <button class="action-btn" onclick="event.stopPropagation(); vorschlagUebernehmen(${{i}}, this)">✓ Übernehmen</button>
        <button class="action-btn" onclick="event.stopPropagation(); vorschlagKopieren(${{i}}, this)" title="Falls der Server nicht eingerichtet ist">📋</button>
      </td>
    </tr>`;
    return tr;
  }}).join('');
...
async function vorschlagUebernehmen(index, btn) {{
  ...
  const erfolg = await speichereDauerhaft(v.ticker, vorschlagFelder(v), null);
  if (erfolg) {{
    btn.textContent = '✓ OK';
    btn.disabled = true;
  }} else {{
...
async function vorschlagUebernehmenAusSteckbrief(ticker, btn) {{
  const v = VORSCHLAEGE.find(x => x.ticker === ticker);
  if (!v) return;
  const statusEl = document.getElementById('speicher-status');
