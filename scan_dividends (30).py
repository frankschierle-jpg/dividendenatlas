...
def main():
    ...
    for ticker in all_tickers:
        ...
        # Gesamtrendite 1J: Dividendenrendite + Kursentwicklung der letzten
        # 12 Monate - grobe Naeherung (ignoriert Zinseszins/genaue Zeit-
        # punkte der Ausschuettungen), aber zeigt sofort, ob eine hohe
        # Dividende einen Kursverlust ueberhaupt ausgleicht.
        perf_1y = kurs.get("perf_1y")
        gesamtrendite_1j_pct = (round(dividend_yield_pct + perf_1y, 2)
                                if dividend_yield_pct is not None and perf_1y is not None else None)
        ...
        data = {
            ...
            "perf_1y": perf_1y,
            "gesamtrendite_1j_pct": gesamtrendite_1j_pct,
...
function showDetail(ticker) {{
  ...
  document.getElementById('detail-content').innerHTML = `
    ...
    <table>
      <tr><td>ISIN</td><td style="font-family:monospace;">${{r.isin || 'nicht verfuegbar'}}
      ...
      <tr><td>Gesamtrendite (1 Jahr)</td><td>${{fmt(r.gesamtrendite_1j_pct, '%')}}
        <div style="font-size:11px;color:var(--muted);font-weight:normal;margin-top:2px;">Dividendenrendite + Kursentwicklung der letzten 12 Monate. Zeigt, ob die Dividende einen Kursverlust ueberhaupt ausgleicht - eine hohe Rendite bei stark negativer Gesamtrendite ist meist keine echte Chance.</div></td></tr>
      <tr><td>Ausschuettungsquote${{r.payout_ratio_fcf_pct !== null && r.payout_ratio_fcf_pct !== undefined ? ' (Free Cashflow)' : ' (Gewinn)'}}</td><td>${{fmt(r.payout_ratio_fcf_pct !== null && r.payout_ratio_fcf_pct !== undefined ? r.payout_ratio_fcf_pct : r.payout_ratio_pct, '%')}}
      ...
      <tr><td>Dividenden-Historie</td><td>${{r.dividend_streak_jahre ? r.dividend_streak_jahre + ' Jahre ununterbrochen gezahlt/erhoeht' : 'nicht recherchiert'}}
      ...
      <tr><td>Nächste erwartete Auszahlung</td><td>${{r.next_payout || 'nicht verfuegbar'}}
      ...
      ${{(r.debt_to_equity !== null && r.debt_to_equity !== undefined) ? `<tr><td>Verschuldung (D/E)</td><td>${{fmt(r.debt_to_equity)}}
