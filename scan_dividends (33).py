...
def evaluate_scanner_candidate(f):
    ...
    # FCF-basierte Quote ist robuster gegen Bilanzeffekte (Abschreibungen,
    # Fair-Value-Anpassungen bei Immobilien/Versicherern) als die gewinn-
    # basierte - wird bevorzugt verwendet, wenn vorhanden (optionales Feld
    # in stammdaten.json). Fallback auf die gewinnbasierte Quote, wenn FCF
...
function evaluateScannerCandidateJS(f) {{
  ...
  const yield_ = f.dividend_yield_pct, debt = f.debt_to_equity;
